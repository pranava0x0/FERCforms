...
- **Every commit gets an adversarial code review before merge — even code the same session wrote and tested.** 2026-07-06: a session-authored, session-tested `amount_usd` feature (60/60 citations passing its own `--live` validation) still had 2 reproducible crash bugs and a range-handling bug that silently understated real dollar figures by up to 6 orders of magnitude — all confirmed on real committed data, none caught by the feature's own unit tests. One bug (a decimal-point-as-sentence-terminator error) was only found by *re-running the real pipeline on real data and reading the diff* after applying an earlier fix — not by the passing unit tests. Lesson: a fix passing its own tests is necessary, not sufficient; run it against real data and inspect the output before trusting it.
- **Constructing a model via `**{k: v for k, v in existing.items() if k not in (...)}` + explicit kwargs? Exclude EVERY explicit kwarg from the spread, not just the ones you're replacing.** `pipeline/structure.py` had two functions that spread `existing_report.items()` (excluding only `"findings"`/`"finding_count"`) while also passing `structured=False` explicitly — since every committed `report.json` already has a `structured` key, this raised `TypeError: got multiple values for keyword argument 'structured'` on every re-run against an already-processed record (caught 2026-07-06 by code review, not by the test suite). Any time you add a new explicit override to one of these merge-and-override constructors, add it to the exclusion tuple too.
- **Verify the files you think you committed are IN the commit — `git log -1 --stat`.** 2026-07-16: `tests/test_tokens.py`, the entire deliverable of a phase, matched `.gitignore`'s credential glob `*_token*` and was dropped by `git add -A` with **no warning and exit 0**, while the commit message described it in detail. A fresh clone would have had 188 tests instead of 230 and none of the guard. Broad credential globs (`*_token*`, `*_secret*`, `*_password*`, `oauth*.json`) are correct and should NOT be weakened — **rename the file** (`test_palette.py`) and move on. Any file whose name would trip a security glob is a file git will silently ignore.
- **A colour annotated "AA" in a design doc is a claim, not a measurement — compute the ratio.** 2026-07-16: a hand-written palette shipped three contrast failures at once, all invisible to review. The worst had been live for a while: `color: #fff` on `background: var(--accent)` is 5.37:1 in light mode and **2.73:1** in dark, because the dark accent is a *light* tint — whoever writes the rule is only looking at one theme. The spec's own amber, annotated "AA on paper", measured 4.48:1. **A ~60-line test that parses the token block and computes WCAG ratios for every text/bg pair in both themes found all three in one run**, and it doubles as the "no hardcoded hex outside `:root`" guard (a hardcoded hex is exactly what can't be themed). Any project with a dark mode should have it. Corollary: a filled surface needs an explicit `--on-<role>` foreground token per theme; "white text" is not a theme-independent fact.
- **Never regex-rename across a whole file — it rewrites your prose too.** 2026-07-16, twice in one operation: renaming a fixture `page` → `site_page`, the first pattern's lookahead excluded `.` and so skipped every `page.locator(...)` (the actual usage) while still corrupting comments; the second got the identifiers right and turned docstrings into "a real site_browser", "the 20-card first site_page", "the site_page scrolls horizontally". A regex cannot tell code from English. Use an AST/LSP rename, or scope the edit to identifiers, and **re-read the diff for prose** before committing — this is the AI-slop failure mode reviewers notice first.
- **A local `conftest.py` fixture silently SHADOWS a plugin's fixture of the same name.** 2026-07-16: `pytest-playwright` publishes `page`/`browser`/`context`; naming ours the same made ours win invisibly — a reader can't tell which they get, and deleting ours would hand tests a differently-configured browser that never visits the site. Name project fixtures distinctly (`site_page`, `site_browser`). Run `pytest --fixtures` when adding a fixture with a common name.
