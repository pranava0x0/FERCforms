...
11. ⊕ **[low] Reusable `verify-by-download` helper** — wrap the proven stream-GET + ~45 MB cap + `%PDF-` magic + `fitz` page-1 caption + page_count into `scripts/verify_pdf_url.py` (or a `pipeline.sources --check-url` flag) so source verification is one repeatable command, not a throwaway script that block-buffers and hangs (the 2026-06-22 "Download + verify page 1" exit-144 was a `pkill` of exactly that). Pattern already in the skill + data-sources.md. → *Quality & testing*
