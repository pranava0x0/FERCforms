...
- **[high] Finding taxonomy.** A controlled vocabulary of finding types (accounting misclassification, formula-rate inputs, affiliate transactions, capitalization vs expense, etc.); tag every finding. *(policy 2026-06-01: make **ratepayer harm** a top-level axis — flag the classes that directly overcharged customers (below-the-line lobbying/charity/dues, mispriced affiliate transactions, depreciation/AFUDC over-recovery, excess ADIT owed back) so the site can filter to "findings that cost customers money.")*
...
- **[high] Ratepayer-impact lens (dollar-impact aggregation).** *(promoted from low — policy 2026-06-01.)* Once `amount_usd` is extracted, sum quantified impacts by theme / company / year and lead with a headline stat — e.g. *"$X improperly recovered from customers 2014–2025; $Y of it below-the-line lobbying/charity/dues."* Turns the compliance feed into a quantified over-recovery dataset — the one affordability story this corpus can actually tell. Depends on Section IV parsing above.
