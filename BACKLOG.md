...
- **[med] Phase-2 states — VA / IL / SC / TX (metadata-only).** Open-access but order/testimony-heavy: VA SCC `docketsearch/DOCS/{code}!.PDF` (codes via the JSON Search API), IL ICC e-Docket, SC PSC DMS + ORS, TX PUCT Interchange `Documents/{control}_{item}_{docid}.PDF` (some scanned → OCR). Add a seed per state. See [ISSUES.md](ISSUES.md) for access mechanics.
