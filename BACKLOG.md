...
10b. ⊕ **[high — spec ready, not started] Analyst-grade UX + visual-identity refresh** — placeholder for the full phased spec in [docs/analyst-ux-identity-spec.md](docs/analyst-ux-identity-spec.md) (written 2026-07-16 from a FERC-analyst + general-energy-reader evaluation, plus emp.lbl.gov data-product research). 4 phases: (1) card info-scent + sort + incremental render + payload split; (2) periwinkle "public record" identity (color-theory palette, stamp pills, ledger surfaces); (3) URL state/permalinks + Ledger view + company/theme lenses + data-downloads page + Atom feed; (4) dollar lens + annual "Edition" briefing (gated on amounts scale-out, P1 #4). → *Design / UI*
