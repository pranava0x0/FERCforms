...
## ⭐ Prioritized queue (reprioritized 2026-06-22)
...
0b. ✅ **[DONE 2026-07-06] Reclassified — not purged.** User's call: keep the 84 documents (they're real), but they're not audits of a utility, so give them their own honest home. Added a **5th collection `state_reference`** (`pipeline/build.py` COLLECTIONS + `docs/js/app.js`, parity test green) and moved all 84 `state_puc.json`/`state_puc_tier2_extended.json` records into it with an accurate, categorized `doc_type` (19 TX-PUC-self-audits, 22 DC RPS/fuel-mix filings, 8 PA seasonal-readiness filings, 16 misc admin/reference docs, 6 nav pages, 2 blank forms, 2 tip-line pages, 2 meeting notices, 1 FAQ, 1 guide — verified in the live preview, zero stray "unknown"/generic labels). `state_audit` is now a clean 49-report, 150-finding, audit-only tab. See ISSUES.md 2026-07-06.
...
4. ✅ **[DONE 2026-07-06, pilot scale + code-reviewed] `amount_usd` extraction with citations, piloted on 18 FERC audits** — 60 findings now carry a cited headline dollar figure (`amount_usd`/`amount_usd_quote`/`amount_usd_page`); the design deliberately does NOT re-derive new "verbatim" text from raw PDF prose (the exact anti-pattern the same day's validation sweep fixed elsewhere) — it only finds the first dollar figure already present in a finding's OWN committed summary/recommendations text, at sentence granularity, then locates which source page carries it. Citations are fail-closed: 3 dollar mentions with no locatable page match were correctly left uncited rather than guessed (a body-text line-wrap/rephrasing difference from the committed exec-summary text). **Independently verified**: `pipeline.verify_amounts --live` re-fetched + re-extracted all 18 source PDFs fresh and confirmed all 60 citations — 0 failures. **Post-pilot code review found + fixed 10 issues** before merge (2 crash bugs in the earlier parser fix, a range-handling bug that changed 4 reports' actual `amount_usd`, a decimal-point sentence-splitting bug, plus efficiency/reuse cleanups) — see ISSUES.md 2026-07-06 "code review" entry. New corpus-wide offline regression test (`test_amount_usd_citations_are_self_consistent`) keeps this green going forward. **UI shipped 2026-07-10**: `docs/js/app.js` now renders `amount_usd` as a compact `$` pill on each finding (`fmtAmount` → $3.5M/$818K/$24M) with the verbatim `amount_usd_quote` + source page in the tooltip. **Still open**: scale `amount_usd` to more of the corpus (see `pipeline.amounts_enrich --ids ...`), then the dependent **[high] Ratepayer-impact $ magnitude** rollup — the one affordability story the corpus can tell ("$X improperly recovered from customers"). → *Pipeline & data / Analysis*
...
**P3 — strategic / large (gated on a richer library or a separate module):**
8. **[high] "Audit-my-document" mode** — consumes the pattern library; gated on the findings depth in P0–P1. → *The bigger vision*
9. **[high] New collection: FERC Office of Enforcement actions** — mission-aligned new document type. → *The bigger vision*
10. **[high — large] Second module: forward-looking FERC dockets** + **[med] Form 1 financials via PUDL** / the Form 1 ETL — separate big-scope builds, not refresh increments. → *The bigger vision / FERC Form 1 Analysis*

**P2.5 — user-facing (site) track, parallel to the data track:**
10b. ⊕ **[high — Phase 1 DONE 2026-07-16; Phases 2–4 open] Analyst-grade UX + visual-identity refresh** — placeholder for the full phased spec in [docs/analyst-ux-identity-spec.md](docs/analyst-ux-identity-spec.md) (written 2026-07-16 from a FERC-analyst + general-energy-reader evaluation, emp.lbl.gov data-product research, and a 10-product comparative study — Violation Tracker/ECHO/EDGAR/ProPublica/CourtListener/LegiScan/OpenSecrets/PUDL/DOL/eLibrary — yielding an 11-job JTBD map, 6 core flows, and a per-surface CTA system with hero search + trust line + feeds). 4 phases: **(1) ✅ card info-scent + sort + incremental render + payload split + hero/entry layer — shipped**; (2) periwinkle "public record" identity (color-theory palette, stamp pills, ledger surfaces); (3) URL state/permalinks + Ledger view + company/theme lenses + data-downloads page + feeds + full CTA build-out; (4) dollar lens + annual "Edition" briefing (gated on amounts scale-out, P1 #4). → *Design / UI*

**P4 — supporting / DX / lower (pull when adjacent):**
11. ✅ **[DONE 2026-07-10] Reusable `verify-by-download` helper** — landed as `pipeline.verify_sources --live` + the reusable primitives `fetch_pdf_bytes` (stream-GET + 45 MB cap + browser-UA fallback), `company_tokens`, `content_match_fails`. Re-fetches every fetchable PDF and re-confirms it is STILL the claimed document (page-count == committed report + a company token present in the text), closing the gap where the offline sweep marks a `fetch=true` record PROVEN on the `page_count>0` shortcut without re-checking content. Caught the image-only CA UAB scans (0 extractable text) that would otherwise have shipped blind. Full corpus: 279 verified, 0 mismatches. → *Quality & testing*
12. [med] partial-date support · full-text search · company-response capture · interconnection facet · filtered CSV export · doc-type facet · listing↔PDF integrity check.
...
- **+67 findings across 14 reports** (corpus 1727 → **1794**); FERC zero-finding reports 26 → **12**, and the 12 remaining each *state* they found none (small-entity letters + "A. Conclusion" reports) — the FERC parser-coverage gap is effectively closed.
- **How:** re-fetched all 26 (eLibrary), re-extracted, added an **additive** `recover_zero_finding_specs` to `pipeline/structure.py` that runs ONLY when the primary path finds 0 real findings. It re-extracts with **PyMuPDF** (linearizes multi-column Exec-Summary lists pdfplumber scrambles) and parses four formats: numbered `N. Title – summary`, bulleted `• Title – summary`, the legacy body "IV. Finding(s) and Recommendations" section (findings vs. interleaved recs split by each finding's "Pertinent Guidance" block), and order-style inline `(1)…(2)…` lists.
- **Validation gate:** accept a recovered list only if its count equals the report's own stated count ("Audit staff identified N areas of noncompliance"); a partial/garbled parse is rejected (stays 0), so no wrong verbatim text ships. Recovered, e.g.: SDG&E FA19-3 (11), Idaho PA17-7 (10), PacifiCorp FA16-4 (9), WEC FA21-2 (8), National Grid FA16-2 (7), Dominion FA15-16 (6), CAISO PA17-3 (5), Kinder Morgan FA14-10 (4), KCP&L PA17-4 (2), + 5 singles.
- **Zero regression:** git confirms only the 14 recovered `report.json` changed; the 94 validated reports are byte-identical. Tests: 9 synthetic parser tests + the (previously orphaned, now wired) `structure_snapshot_validated.json` guard. **151 tests pass.**
- **Discovered + logged:** pdfplumber extraction is nondeterministic — a blind whole-corpus re-structure drifts validated reports (Cleco 12→1). Re-structure targeted ids only; new `[high]` backlog item to fix. See ISSUES.md.
...
- **+5 NY DPS comprehensive M&O audits** (all page-1-verified, `fetch=true`, PROVEN; corpus 655→660): two NorthStar audits — **NYSEG/RG&E** Case 23-M-0103 (439-pp final report / 128 recs + initiating order, releasing order, 716-pp implementation plans) and **LIPA/PSEG-LI** Matter 21-00618 (569-pp final report / 49 recs). `data/seeds/ny_dps_audits.json`. Commits `9f44c17`, `53d7abd`.
- **FERC non-gap confirmed (2026-06-22):** no audit reports issued since FA24-3 (2026-05-07); all FY25/FY26 `FA*/PA*` hits are "commencing an audit" start notices. Recorded in `docs/data-sources.md`.
- **`docs/AUDIT_THEMES.md` regenerated faithfully from `patterns.json`** — replaced a fabricated theme set (PROCUREMENT/PENSION/TAX/INSURANCE/… with invented counts) that never matched the pipeline's THEME_RULES; now auto-generated with the real 25 keyword-mined themes + true counts.
- **Tooling/docs:** cracked the **scriptable NY DPS DMM doc-list** (`/search/Home/DocumentSearch2/?searchCriteria=`, supersedes "browser-harvest"); added the **verify-by-download** pattern to the `refresh-ferc-data` skill. See `docs/data-sources.md`.
- **Finding (parked as P0 above):** ~166 WA+IN `state_audit` records are crawler junk; real parsed findings concentrate in PA(11)/NJ(6). See [memory: corpus-depth-vs-breadth].
...
- **[med] Systematic fuel-adjustment-clause / ERRA fuel-cost prudence harvest across states.** *(Added 2026-06-22.)* Only **18** fuel/FAC-prudence records today, yet every state with a fuel-cost-recovery clause runs an **annual** prudence/reconciliation proceeding (CA ERRA, SC fuel dockets, AR FR, KS RECA/ACA, IL/IN FAC/GCA, …) — the most *recurring, on-theme* (cost recovery) document type a state corpus can hold, and a natural recurring-refresh cadence. Many sources are already cracked (SC DMS, AR olsv2, KS estar, CA `PublishedDocs`, IL e-Docket) — this is a focused breadth wave: one fuel docket per electric utility per state-year, verified page-1, metadata-only. Pairs with the dollar-impact lens (a fuel disallowance is a clean `$` figure).
- **[med] "Make one state complete" — a depth showcase (CA or NY).** *(Added 2026-06-22.)* Rather than thin coverage everywhere, take one major jurisdiction end-to-end across **all** its on-theme doc types: comprehensive M&O audit + base-rate case + fuel/ERRA prudence + reliability/service-quality + any billing/enforcement investigation. NY (now 14 records incl. 2 NorthStar M&O audits) and CA (CPUC ERRA + balancing-account audits) are the strongest candidates. Demonstrates what "complete" looks like and stress-tests the multi-collection model on one well-understood state before scaling the pattern.
...
- **[DONE 2026-06-23 — P0 #2] Recovered findings from 14 of the 26 zero-finding FERC reports.** Additive `recover_zero_finding_specs` in `pipeline/structure.py` (PyMuPDF re-extract + 4 format parsers + stated-count validation gate), gated so the 94 validated reports never enter it. **+67 findings, corpus 1727 → 1794**; the 12 still at 0 each *state* they found none (genuinely clean). Tests: 9 synthetic + wired snapshot guard. See the Done section above + ISSUES.md. **Spawned [high] item below (extraction determinism).**
- **[high] Make PDF text extraction deterministic (or the exec-vs-TOC choice robust to a stray "1.").** *(Discovered 2026-06-23 during P0 #2.)* Re-extracting an already-validated report with pdfplumber yields slightly different text that can re-structure to a different finding count (Cleco FA18-3 12→1, Kern FA23-10 6→5 — a spurious "1." in the exec summary wins over the TOC fallback). So a blind whole-corpus `extract`+`structure` silently drifts validated reports; today the rule is **re-structure targeted ids only**. Fix the nondeterminism (pin extractor behavior) or make `structure_report`'s exec-summary-vs-TOC preference robust to a single stray leading number, so a full rebuild is safe and reproducible. Guarded for now by `tests/test_structure.py::test_committed_ferc_finding_counts_match_snapshot`. → *Pipeline & data*
- **[OLD — superseded by the DONE above] Recover findings from the 26 zero-finding reports.** A corpus audit (2026-05-31) found **26 of 120 reports (22%) structure to 0 findings** despite being born-digital and text-extractable. The site now renders an honest *"No findings extracted — read the source PDF"* state on these (so they no longer look like broken empty cards); **this item recovers the underlying data**, which is the whole "more documents" lever now that FERC has published no new audits since Sept 2025 (re-verified 2026-05-25). Two tracks, **both gated by a no-regression snapshot test** of current per-report finding counts so the 42 validated reports cannot break (the documented overfit trap: Cleco 12→1, MISO 3→7):
...
- **[✅ PHASE 1 DONE 2026-07-16 — `c498b53`] Analyst-grade UX spec, Phase 1: analyst quick wins + perf floor + entry layer.** Shipped: **B2** payload split — `reports_index.json` (card/filter fields + `rec_count`/`amount_max` rollups) + lazy per-collection `findings_<collection>.json`; first-paint data **1.96 MB → 295 KB raw** (index 241 KB / 25 KB gz), losslessly reversible (`split_reports`/`merge_split` + a corpus round-trip parity test); stale 3.4 MB `reports_pre_cost_extraction.json` deleted. **B1** 20-card IntersectionObserver paging + threads built on first expand only. **A2** card scent (mechanical subject, 3 theme chips +N, report $ pill, amber harm tick) and the **F2** "0 recs" fix. **E1** hero search + mechanically-derived corpus trust line (`meta.jurisdictions_covered` added to the bake), year presets anchored on the corpus's newest year, "See an issue? Report it". **A3** sort, **A11** 150 ms debounce, **B3** phone bottom bar + tablet left panel + wrap/date fixes, **B4** pill sizing + retry states. 188 tests pass. **Still open from the spec:** Phase 2 (identity), Phase 3 (permalinks/ledger/lenses/data page/feeds), Phase 4 (dollar lens/Edition briefing). Deferred decision: `cross_links.json` (363 KB, still baked, still consumed by nothing) — surface it in the A4 company lens or stop shipping it.
- **[high — SPEC READY 2026-07-16; Phase 1 shipped, Phases 2–4 open] Analyst-grade UX, interaction model & visual identity refresh.** Full spec: [docs/analyst-ux-identity-spec.md](docs/analyst-ux-identity-spec.md). Evaluated the live site at 375/768/1280px from two personas (FERC/regulatory analyst; general energy-sector reader), mined LBNL EMP (emp.lbl.gov) data-product conventions, and ran a comparative CTA/flow/JTBD study of ten enforcement & civic-data explorers (Violation Tracker, EPA ECHO, SEC EDGAR FTS, OSHA/DOL, FERC eLibrary as the pain baseline, ProPublica Nonprofit Explorer, CourtListener/RECAP, LegiScan, OpenSecrets, PUDL) — distilled into an 11-job JTBD map (§1.1), six core flows with acceptance tests (Part E2: orient, research-a-pattern, background-a-company, cite-&-share, take-the-data, monitor), and a per-surface CTA system (Part E1: hero search + corpus trust line, question-card router, source-triplet thread CTAs, "Get the data"/"About the data"/"What's new" nav, RSS follow CTAs; hard rules: no login walls, no export gates, no multi-field entry forms). 15 current-state findings (F1–F15), the load-bearing ones: collapsed cards carry no information scent (no subject/themes/$); the "0 recs" pill misreads as "no recommendations"; no shareable URL state/permalinks; whole-corpus DOM render janks scrolling; reports.json (1.9 MB raw) is the first-paint payload; stale 3.4 MB `reports_pre_cost_extraction.json` + unconsumed `cross_links.json` ship in docs/data; no sort, no compact ledger view, no company lens; identity reads generic-AI-pastel and rec-blue collides with brand periwinkle. Spec keeps periwinkle as anchor and derives the full palette by hue relationships (finding=near-complement amber, rec=split-complement teal, categorical wheel for charts), system-stack typography with mono dockets + serif finding titles, "stamp" pills, ledger surfaces. 4 phases with acceptance criteria; Phase 4 (dollar lens, annual Edition briefing) gated on the amounts scale-out (P1 #4). Absorbs/updates existing items: filtered CSV export + stable cite URLs, doc-type facet insight, per-report pages, accessibility/Lighthouse pass, "editorial + periwinkle" variant (this spec IS that variant, applied as default).
- **[high — NEXT] Analyst-UX spec Phase 3, remainder: A7 · A10 · E1 CTA build-out.** Shipped so far: A1 permalinks (`34d42ff`), A5 theme panel + A4 company lens (`69afe96`), A3 ledger (`f2bbb0f`). **Left:** **A7** a `data.html` access-mode matrix (bulk JSON / CSV / llms.txt / per-record), a data dictionary covering every `findings.csv` column, suggested citation, and a changelog; plus a **filtered** CSV export in the toolbar (export what you're looking at, not the whole corpus — the Violation Tracker pattern). **A10** `feed.xml` (global + per-collection + per-company) from `build.py`, with **stable GUIDs** (the report id, never a hash of mutable fields) — the no-account monitoring path. **E1** slim header nav (Get the data · About the data · What's new — provenance-as-navigation), and the OpenSecrets-style question-card router ("Has my utility been audited?" → Search by company). Acceptance for each is in the spec's Phase 3 block; flows F-B…F-F have interaction budgets to meet.
- **[high — needs Pranava, blocks nothing] Phase 2 identity: the "doesn't feel like Claude" sign-off.** The palette/type/surface work is built, verified and committed (`e17f01a`); the one acceptance criterion left is a human aesthetic read, which the spec explicitly reserves. Before/after screenshots at 375/768/1280 × light/dark were produced. If the answer is "still too soft", the two levers are (a) C2's deferred self-hosted serif (~35 KB Source Serif subset, behind a measured before/after — the decision gate is written and untriggered), and (b) pushing the surface further toward ledger.
- **[✅ PHASE 2 BUILT 2026-07-16 — needs the user's visual sign-off] Analyst-UX spec Phase 2: "public record, periwinkle" identity.** C1 palette derived by hue relationship from the periwinkle anchor (finding = near-complement ochre, rec = split-complement **teal** — retires the 212° steel blue that read as brand at dot size (F15); new `--rule`, `--cat-1…5`, `--on-accent`, `--on-status` tokens). C2: serif finding titles, mono dockets/$ figures, uppercase eyebrows. C3: hairlines over boxes, radius 12→8, no resting shadow, **stamps** replacing pastel pills (outline default; only the two semantic states fill), square monogram replacing the circle avatar, categorical chart bars. **`tests/test_palette.py` is the new durable guard** (+42 tests → 230): WCAG ≥4.5:1 on every text/bg pair in both themes, no hardcoded hex outside `:root`, ≥45° semantic-vs-brand hue distance. It immediately caught a **pre-existing dark-mode bug** (white on light-periwinkle `.pill.solid`/skip-link = 2.73:1) and that the spec's own `#A8690A` amber missed AA at 4.48:1. **Open:** the user's "doesn't feel like Claude" check — before/after screenshots at 375/768/1280 × light/dark. C2's self-hosted-serif gate remains deferred (not triggered).
- **[med] Filter-chip touch targets miss the 44px floor (fold into the spec's Phase-2 B4 a11y pass).** CLAUDE.md calls ≥44px "non-negotiable on touch devices", but `.filter-chip` is ~28px and `.preset-chip` ~32px, and both live inside the mobile bottom-sheet / tablet slide-in — finger-only surfaces. Pre-existing for `.filter-chip` (Phase 1 only added `.preset-chip` alongside it at the same scale, and raised `.sort-select` to 44px across the whole ≤1023px touch band). Fixing chips in isolation would leave them visually inconsistent with the pill scale, so do it with B4's pill-size/contrast pass, not before. Caught by code review of the Phase-1 diff (2026-07-16).
...
- **[low] Reusable `verify-by-download` helper (kill the throwaway verify scripts).** *(Added 2026-06-22 — the "fix" for the exit-144 "Download + verify page 1" failed task.)* Source verification is currently ad-hoc one-off scripts that **block-buffer stdout and have no size cap**, so a large-PDF download looks like a hang and gets `pkill`-ed (exit 144 — what the screenshot showed). Wrap the proven pattern — streaming `requests.get` + ~45 MB cap + `%PDF-` magic check + `fitz` page-1 caption + `page_count`, with `flush=True`/`python3 -u` — into a committed `scripts/verify_pdf_url.py` or a `python3 -m pipeline.sources --check-url <url>` flag, so checking a candidate source is one repeatable, non-hanging command. The pattern is already documented in the `refresh-ferc-data` skill and `docs/data-sources.md`; this just makes it a tool. Pairs with the existing `pipeline.verify_sources` sweep (that checks *seeded* records; this checks a URL *before* seeding).
