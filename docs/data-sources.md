...
- **WY — PSC · DMS portal (deferred).** `psc.wyo.gov`'s orders link out to the **`dms.wyo.gov/external/publicusers.aspx`** Docket Management System (search portal, no static PDF path surfaced). On-theme: Rocky Mountain Power **ECAM** (Schedule 95 energy-cost adjustment = fuel) — Docket `20000-671-ER-24`. Harvest the DMS download URL (browser) before seeding.
- **HI — PUC · DMS (deferred).** Hawaiian Electric's **ECRC** (Energy Cost Recovery Clause = fuel/IPP cost) is the signature on-theme proceeding (D&O No. 40044, June 2023); documents live in the HI PUC DMS (`dms.puc.hawaii.gov`, `.hawaii.gov` ✓) — needs the DMS document-URL pattern (utility-site copies are on `hawaiianelectric.com`, non-gov). **VT — PUC:** orders are in **ePUC** (`epuc.vermont.gov`) or on the utility site (`greenmountainpower.com`, non-gov); `puc.vermont.gov` static PDFs are mostly plans/procedures, not case orders — defer to ePUC harvesting.
