# Clean Water Act enforcement (energy/utility) & a data-center read-across

> **Outside the FERC audit corpus.** This is web-sourced research, **not** part of the
> verbatim FERC audit dataset (`data/`, `docs/data/*.json`). The Clean Water Act (CWA) is an
> EPA/DOJ statute, a different jurisdiction from FERC. Nothing here is mined, scored, or baked
> into the explorer's findings — it is a standalone reference compiled from primary `.gov`
> sources (EPA, DOJ) plus the Congressional Research Service. Captured 2026-06-02.
> Scope: **energy/utility sector**, **2015 → present**, criminal + civil/administrative +
> landmark. Data-center application is forward-looking analysis (no data-center CWA case exists yet).

## Why this exists

A user asked for "every historical example of a CWA investigation since 2020," later widened to
**2015** and to include **civil** actions. The FERC Audit Explorer holds zero CWA content
(confirmed: 0 matches in `reports.json`), so this was compiled separately. See the project
[BACKLOG.md](../BACKLOG.md) "External research — Clean Water Act (CWA) enforcement" note.
...
| 2015* | **ExxonMobil Pipeline** | Civ | CWA §311 — 2012 North Line rupture, ~2,800 bbl crude, Torbert, LA | **$1,437,120** civil penalty (*date not independently confirmed this pass — DOJ page 403'd*) | [DOJ](https://www.justice.gov/archives/opa/pr/exxonmobil-pipeline-company-pay-civil-penalty-under-proposed-settlement-torbert-louisiana-oil) |
...
Energy/utility NAICS to filter on: 2211 (electric power), 2212 (gas distribution), 211 (oil & gas
extraction), 4861/4862/4869 (pipeline transport), 32411 (refineries), 2121 (coal mining).

### Leads surfaced but NOT yet confirmed against a primary source
- **Energy Transfer** — reportedly ~$7M federal CWA penalty for three Louisiana releases (+ ~$1M LA state, Caddo Parish). Couldn't locate the primary `.gov` page this pass.
...
## Why these actions were brought — the CWA hooks, and how they map to data centers

Because **no data-center CWA case exists yet**, the data-center column is a forward-looking
read-across from the statutory hooks the energy cases turned on — analysis, not enforcement.
...
- **Cooling/process water (the §402 NPDES analog).** Evaporative-cooling **blowdown** carries biocides,
  anti-scalants, and **heat** (a regulated pollutant); discharging to surface water (or some POTWs)
  implicates NPDES numeric limits — the *West Penn* pattern.
...
broader-than-federal water programs**, so a "no federal jurisdiction" wetland can still be fully
state-regulated. Net: *Sackett* narrows the §404/§401 wetlands path, not the operational-discharge or
spill paths that drove most of the cases above.
