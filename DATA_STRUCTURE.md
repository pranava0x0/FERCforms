...
| `state_audit` | **State PUC Audits** | State PUC/PSC/SCC audits (VA, TX, IL, SC, OH, PA, MI) | **Partly** — PA M&O audits parsed (`pipeline/state_structure.py`); the rest metadata-only |

`collection`, `jurisdiction`, `source`, `doc_type`, and `structured` are fields on `AuditReport`, defaulted to the original FERC-audit identity so the 120 committed reports validate unmodified. Each record names its government issuer in `source` — FERC audits carry `"FERC Office of Enforcement, Division of Audits & Accounting"` (`structure.FERC_AUDIT_SOURCE`); state records carry their commission name. A corpus-wide test asserts every committed `report.json` has an official-`.gov` `source_page_url` + `pdf_download_url`.
...
**Two more `SourceSeed` flags.** `parse` (default `False`) and `fetch` (default `True`):
- **`parse=False` (metadata-only) is the default.** The document is captured with its source link + full provenance but **not** machine-extracted into findings — FERC prudence orders are adversarial legal opinions / settlements (free-form prose, no findings list), and most state docs are orders/testimony. The **exception (`parse=True`, live):** PA PUC **Management & Operations** audits, parsed verbatim from their Exhibit I-2 "Summary of Recommendations" (`pipeline/state_structure.py`, snapshot-gated). Each parsed `Recommendation` also carries **`source_page`** — the printed body page the Exhibit I-2 "Page No." column cites (where the rec is discussed) — surfaced in the UI as a `(p. N)` chip and in `llms-full.txt` as `(source p. N)`. `source_page` is `null` for sources without a page-tagged recommendation index (e.g. FERC executive summaries). Only flip `parse=True` for a format with a clean enumerable structure + a no-regression test; a parse miss falls back to metadata-only so garbled "verbatim" text never ships (the [quote discipline](AGENTS.md)). Remaining formats (PA focused/MEI, MI Liberty) stay metadata-only — a [BACKLOG.md](BACKLOG.md) item.
- **`fetch=False`** ⇒ don't machine-fetch: the PDF URL was captured **out-of-band in a real browser** because the source is WAF-blocked (OH PUCO F5, NC NCUC Cloudflare). The record is written metadata-only straight from the seed (page_count 0), avoiding doomed requests to the blocked endpoint.

The site renders metadata-only records with an honest **"Listed for reference"** state (`structured=false`) and a link to the source PDF — distinct from a genuinely finding-free audit ("No findings extracted").

**Seeded so far (26 state + 5 prudence).** State: **VA** SCC (5 rate/cost-recovery orders), **TX** PUCT (4, El Paso Electric fuel reconciliation), **IL** ICC (4, ComEd Rider CFRA reconciliation), **SC** PSC (3, Dominion Energy SC fuel-cost review), **OH** PUCO (2, FirstEnergy political/charitable-spending review, `fetch=false`), **PA** PUC (4 Bureau of Audits M&O/MEI; the 3 M&O are **parsed**), **MI** MPSC (4 Liberty Consulting U-21305 audits). Prudence: FERC eLibrary formal-challenge orders + an ALJ initial decision (2015–2025). **Per-regulator access mechanics** (scriptable vs. WAF-blocked, URL patterns, gotchas) are in **[docs/data-sources.md](docs/data-sources.md)**; live access status in [ISSUES.md](ISSUES.md).
